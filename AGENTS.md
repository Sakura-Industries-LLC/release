...
Agent sessions for this repository run from the parent meta repo,
`Sakura-Industries-LLC/dntls-lab-root`, which checks this repo out as the
`release/` sub-directory. There is no session protocol here: do not start,
continue, or close sessions in this repo, and do not install one.

Session history for this repo will live in the meta repo's journal branch
under `.journal/repos/release/`.

If you are running directly inside this repo, `cd ..` to the meta repo root
before any session lifecycle command.
